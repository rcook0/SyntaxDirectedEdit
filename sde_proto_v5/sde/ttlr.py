...
class LRParser:
    def __init__(self, tokens: List[Token]):
        self.tokens = tokens
        self.shift_nodes: List[ASTNode] = []  # terminals in shift order
        ...
        while True:
            ...
            if act is None:
                if t.type == 'EOF' and state in (9,):
                    ...
                    continue
                raise SyntaxError(f"Unexpected token {t.type} at {t.start} in state {state}")
            kind, arg = act
            if kind == 's':
                ...
                if goto is None:
                    raise SyntaxError(f"No GOTO from state {state2} on {lhs}")
                stack.append((goto, node))
            elif kind == 'acc':
                root = make_node('ROOT', [stack[-1][1]])
                # Attach shift_nodes for fast leaf access downstream
                setattr(root, "_shift_nodes", self.shift_nodes)
                root.rebuild_threads()  # safety; we still build full threads for now
                return root
